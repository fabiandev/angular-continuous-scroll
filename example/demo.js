(function(angular) {
  ...
  angular.module('Demo')
    .controller('MainController', function($scope, $location, $timeout, PostLoader) {
      ...
      // Scope methods
      $scope.nextPage = function nextPage(event, EndlessScroller) {
        $scope.loading = true;

        postLoader.next().then(onPageLoad, onPageLoadError);
      };

      $scope.previousPage = function previousPage(event, EndlessScroller) {
        $scope.loading = true;
      ...
      function updatePage(event, EndlessScroller) {
        ...
        $location.search('page', EndlessScroller.currentPage);
      };
